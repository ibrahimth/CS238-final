...
def run_LSTM(train_data, test_data, model='test', save_path=None, numEpochs=None, train_only=False): 
    ...
    config.vocab_size = len(set(train_data[1].flatten()))
    eval_config = get_config(model)
    eval_config.batch_size = 1
    eval_config.num_steps = num_steps #1
    eval_config.vocab_size = len(set(train_data[1].flatten()))
    all_test_x = []
    all_test_y = []
    with tf.Graph().as_default(), tf.Session(
        ...
        for i in range(config.max_max_epoch):
            ...
            if train_perplexity <= 1.0 + tolerance:
                break
        end = time.clock()
        timeTrain = end - start
...
#test data needs to be (X, y)
#X of shape (numInputs=1(probably), traj_len, numFeatures)
#Y of shape (numInputs=1, traj_len, 1), but do not care what Y actually is
def run_LSTM_testonnly(X, model_load_path, model="128x2", num_classes=3):
    num_inputs = X.shape[0] #probably 1
    traj_len = X.shape[1]
    in_size = X.shape[2]
    out_size = 1
    print(X.shape)
    y = np.array([0] * num_inputs * traj_len * out_size)
    y = y.reshape((num_inputs, traj_len, out_size))
    test_data = (X, y)
    num_steps = traj_len - 1
    if "LSTM" in model:
        model = model[len("LSTM_"):]
    ...
    with tf.Graph().as_default(), tf.Session(
        ...
        tf.initialize_all_variables().run()
        '''
        new_saver = tf.train.import_meta_graph(model_load_path)
        new_saver.restore(session, model_load_path[:-len(".meta")])
        with tf.variable_scope("model", reuse=None):        
            mtest = Model(is_training=False, config=eval_config, 
                          input_size=in_size, output_size = out_size)
        tf.initialize_all_variables().run()

        p_dists, _, _ = run_epoch(session, mtest, test_data, tf.no_op())
    return p_dists
    
def get_vars_to_test(model_load_path, in_size=9, out_size=1, traj_len=10, model="128x2", num_classes=3):
    num_steps = traj_len - 1
    if "LSTM" in model:
        model = model[len("LSTM_"):]
    eval_config = get_config(model)
...
#data is of shape num_frames, num_features + 1
def isEmpty(data):
    for frame in data:
        if frame[0] > 0:
            return False
...
#test_x is of shape (numFids, 1, numFeatures + 1) = (numFids, numFeatures + 1)
def trim(test_x, test_y):
    start = -1
    end = len(test_x)
    for i in range(len(test_x)):
        frame = test_x[i]
        if frame[0] == 1 and start == -1: #check if the indicator is 1 (used)
            start = i
        elif frame[0] == 0 and start >= 0: #check if first not used
            end = i
            break
...
def data_iterator(raw_data, batch_size, num_steps):
  ...
  # num_steps is the number of frames to give
